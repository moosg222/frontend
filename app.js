const API_BASE = 'https://backend-twlq.onrender.com'; // <- cambiar por la URL de Render o localhost:4000/api
...
if (document.querySelector('#create-act') || document.querySelector('#logout')) {
  // check auth
  if(!getToken()) { location.href = '/frontend/index.html'; }
  ...
  async function loadActivities(){
    const res = await fetch(`${API_BASE}/api/activities`, { headers: { 'Content-Type': 'application/json', ...authHeaders() }});
  ...
  window.deleteAct = async (id) => {
    ...
    const res = await fetch(`${API_BASE}/api/activities/${id}`, { method:'DELETE', headers: { 'Content-Type':'application/json', ...authHeaders() }});
    if(res.ok) loadActivities(); else showMsg('Error al borrar');
  };

  window.openInteract = async (id) => {
    const res = await fetch(`${API_BASE}/api/activities/${id}`, { headers: { 'Content-Type':'application/json', ...authHeaders() }});
